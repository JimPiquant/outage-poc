...
- **2026-04-28:** Fixed TM probe path mismatch (root cause of `fallback-afd` showing `Degraded` in Amos's failover demo). Picked **Option B** (no Bicep change) over Option A. Verified TM profile config: `monitorConfig.path = /outage-poc/health` is correct as-is, since the GH Pages primary lives under `/outage-poc/` and can't be moved to apex. Live confirmation: primary `/outage-poc/health` → 200, AFD/SWA `/health` → 200, but AFD/SWA `/outage-poc/health` → 404 (that's the bug). Contract handed to Alex via `.squad/decisions/inbox/naomi-tm-probe-path-fix.md`: add a second route in `sites/fallback/staticwebapp.config.json` mapping `/outage-poc/health` → `/health.html` (text/plain, no-store). No `what-if`, no redeploy. `monitorStatus` was returned as `null` by `az network traffic-manager endpoint show` (API quirk), but the profile-level `profileMonitorStatus` reads `Degraded` — that's the ground truth and matches Amos's finding. Will go `Online` after Alex's SWA route lands and TM completes one probe window (~2.5 min).
  - **Why Option B:** TM `monitorConfig.path` is profile-wide (one path for all endpoints) — it's not per-endpoint configurable, so any "make Bicep more flexible" approach is a dead end. The only path the GH Pages primary can serve sits under `/outage-poc/`, so the SWA has to match. Zero infra-side risk this way.
  - **Files touched:** none in `infra/`. Added `.squad/decisions/inbox/naomi-tm-probe-path-fix.md`.
